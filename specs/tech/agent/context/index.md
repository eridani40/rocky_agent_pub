...
| **system_reminder** | 动态上下文（todo / squad_task / squad_agents_status 动态半 / tool_error），注入最后一条触发 message（user/tool/a2a）末尾（保 cache）；静态项（env/workspace/团队盘）已迁 system prompt `session_states` mapper（v0.0.361） |
...
| 11 context EP（8 ordered + 3 exclusive）+ 53 rocky_context impl（含 v0.0.49 `store_sink` + v0.0.51 post-compact + v0.0.126 `search_indexing` + v0.0.256 `bubble_text_before_tool_call` + v0.0.361 `session_states` mapper，整合索引） | EP cardinality / plugin 框架（→ `../../plugin_system/`） |
