...
| `rules.md` | 3 section（`# Operating Rules` / `# Doing Tasks` / `# Tool Use`）：系统层（注入可疑 flag / 工具失败不重试 / 自动压缩历史）+ 任务层（反 stub-stop / 反 fabricate / 改文件前先读 / 失败诊断根因 / 不超范围改码）+ 工具层（优先 dedicated tool / 并行调用纪律 / 引用文件路径或 URL 用 markdown 链接语法 `[文本](路径或URL)` 不输出裸路径——前端可渲染可点击，分发见 chat-page `section-preview-area.md`） | ≤ 20 行（调研 §4.2） |
