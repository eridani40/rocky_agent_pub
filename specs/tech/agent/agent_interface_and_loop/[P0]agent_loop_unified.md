...
3. **LLM 调用 + ingest**：main run 先 `await refreshRuntimeConfig(spec, deps)`（v0.0.351：Prepare 后、callLLM 前重读 session 最新 providerId/modelId/effort/approvalMode——模型对变才 `buildLlmClient` 重建 client，不变保持引用稳定；effort/approvalMode 纯值覆盖；不重跑 buildSessionConfigFromDeps；非致命错误 log warn 沿用旧 config；刷新后复查 controller.aborted；旁路 run 跳过，`loop-runtime-config.ts`）→ `base.callLLM`（RunSpec 字段透传 backgroundPath/stop/eosStripper）→ ingest assistant → `onUsage` → fire-and-forget `tryCompact`。
...
**mateExitNotify 装配（mate run 退出通知 leader hook）**：`buildRunDeps` 仅对 `isMain && kind.role==='mate' && kind.derivation==='parent' && opts.deliverToFn && opts.config.sessionContext?.squadId` 装配 `mateExitNotify = { squadId }`（其余 → undefined，全链路 noop 零通知）。**触发过滤语义**：leader（role≠mate）/ subagent（derivation≠parent）/ 非 squad（无 squadId）/ 旁路 run（非 isMain）全部不装配 → 零通知。**执行**：onRunEnd（persistRun/CAS/replySettle 之后）带 `state.stopReason`（6 种正常：no_tool_call/no_new_messages/max_iterations/doom_loop/error/tool_pending）；onInterrupted（settle 之后）固定 `interrupted`——7 种 stopReason 全覆盖。**通知内容**：`state.lastAssistantContent` block 摘要（type ∈ {text, tool_call, tool_result, tool_reply, image}，排除 reasoning/usage；每块前后各 500 截断）+ 耗时（RunLifecyclePort 构造记 startedAt → 退出 diff）+ tool_pending 时读 `Session.pendingToolCalls` 摘要。**退出原因行（`[v0.0.338]` 条件追加）**：`stopReason === 'interrupted'` → `退出原因: interrupted（由用户主动中断，无需处理）`（老板钦定文案）；其余 6 种 reason（no_tool_call/no_new_messages/max_iterations/doom_loop/error/tool_pending）输出 `退出原因: <reason>` 逐字节不变。**投递形态**：Message 仿 send-message-tool 信封（`sender.source='agent'` + `selfAgentRef` + `needReply:false`）→ `deliverTo(leaderSid, msg)`；leader sessionId 两跳解析（`squadStore.getSquad(squadId).leaderId` → `memberStore.getMember(squadId, leaderId).sessionId`，仿 resolveSquadAlias 'leader'）；失败双层 try/catch 仅 warn 不阻断主链。**run-end 汇报去重（v0.0.362）**：leaderSid 解析后、消息构造前判定 `hasRecentLeaderReport` 纯函数——最近 3 轮 assistant message（`snapshot.messages` 过滤 assistant + `lastAssistantContent` 非空拼伪消息补末轮，合并后 `.slice(-3)`）内存在 `send_message` 工具调用且 target 命中 leader（`'parent'` / 显式 sessionId / AgentRef.sessionId 三形态）→ log + return 跳过投递（认定已汇报过；只看 tool_use 存在性，不看 needReply/发送成败；name 形态 AgentRef 不解析比对 = 已知限制，宁多发不漏发）；未命中路径行为不变。机制语义权威（块过滤 / 两跳解析 / needReply=false）见 `mate-exit-notify.ts` + `../../multi_agent/[P1]a2a_protocol.md §4.2`（系统代发兜底同款投递范式）。
