...
10. **[v0.0.139] SessionWorkspaceManager 懒监听（目录级非递归 watcher + tab 监听列表 + 引用计数；取代 v0.0.85 递归模型）**——监听集合 = workspace 根一层 + 当前所有展开目录各一层（chokidar `depth:0` 非递归）。展开=`watch(sid,clientId,workspaceDir,relDir)`、收起=`unwatch`、tab 消失回收：两层触发（① 前端显式 `releaseTab` 优雅回收；② `session_panel` 订阅 1→0 经既有 unsubscribe 钩子 `recycleSession` 兜底）。监听成本与树大小彻底解耦（`.venv` 不展开零成本，扫描风暴结构性消失），**不再依赖 ignore 名单兜性能**。tab 身份=前端 `clientId`（非复用 SSE subId）；目录 refcount 合并多 tab（只 1 物理 watcher）；watch/unwatch 幂等（增量非声明式）；GET tree **绝不隐式 watch**；同 (sid,absDir) create/close **串行化防重入**（Bun FSEvents close 竞态段错误面）。保留 `await waitForChokidarReady(5000)`（超时 resolve 不抛，导出供 UT 直测）；**移除** addDir→`watcher.add` 自动递归（懒监听下新子目录只 emit addDir 显示、不自动 watch，展开才 acquire）。→ `session_workspace_manager.md`
...
| `session_workspace_manager.md` | **[v0.0.139]** SessionWorkspaceManager 懒监听（目录级非递归 watcher + tab 监听列表 + 引用计数 + 两层回收 + 100ms debounce） | [link]([P0]session_workspace_manager.md) |
