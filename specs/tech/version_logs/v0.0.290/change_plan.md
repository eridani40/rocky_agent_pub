# v0.0.290 变更计划书 v2 — fs-yield singleton library（改 import 不改业务代码）
...
**与 v1 的唯一区别**：library 名从 `fs-gate` 改为 `fs-yield`，函数名从 `fsGate()` 改为 `acquireFsSlot()`（语义更准确：获取一个 fs 操作槽位），文档强调 singleton 形态和可复用性。
