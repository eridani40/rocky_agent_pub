...
> 前端打开分流用：`openLocalPath` 文本分支（12 格式 + code）打开前 stat 判定大小——`>5MB` 系统打开 / `≤5MB` 内置编辑（csv/tsv 与图片分支不 stat）。**只 statSync 不读文件内容**（>5MB 大文件先读再判大小 = 本末倒置）。技术权威 `specs/tech/version_logs/v0.0.339-file-open-strategy/change_plan.md` A1。

| 方法 | 路径 | 语义 | 请求体 / query | 成功响应 |
|------|------|------|--------|---------|
| `GET` | `/session/:id/workspace/stat` | workspace 文件 stat（大小判定用；只 stat 不读内容） | query `path`（相对 workspaceDir） | `200` + `{ size: number }` |

```typescript
// GET query param
path: string;   // 相对 workspaceDir 的路径（同 §2.6.1 tree node.path / §2.6.2 OpenBody.path / §2.6.7 file read path）
...
1. method 非 `GET` → `405`（Allow: GET）。
2. `deps.store.getSession(id)` 未命中 → `404`。
3. query `path` 缺失 / 空串 → `400` `{ error: 'path required' }`。
4. 路径解析**复用 `resolveWsFilePath` 安全链**（`realpathSync(workspaceDir)` 根 + `whitelistResolve` 白名单）——穿越（`../` / 绝对路径）→ `400`；白名单 not_found → `404`（与 §2.6.7 file read 同一安全面，不新写路径解析）。
5. `statSync(absPath)` 成功 → `200 { size }`；**目录 → 404**（stat 端点只服务文件大小判定）；stat 失败（不存在/无权限）→ `404`。
6. `workspaceDir` 缺失 / realpath / stat 异常 → `500`。

**示例**：
```bash
curl "http://127.0.0.1:3710/session/01KV.../workspace/stat?path=docs/big.md"
# → 200 {"size":6291456}
```

> 确定性契约，AT 覆盖见 `tests/api/workspace/workspace_stat_tc4/case.yaml`（v0.0.339 新增，5 态断言：200 size / 404 不存在 / 400 越界 / 405 非 GET / 400 path 缺失）。
