...
interface SquadDetail {
  ...
  description: string;
  modelDefault: string;
  // [v0.0.347] 模型路由方案挂载（引用 model_routing_plans planId；未挂载无字段省略——对齐「无 null 输出」）
...
| 方法 | 路径 | 语义 | 请求体 | 成功响应 |
|------|------|------|--------|---------|
| `PATCH` | `/squad/:id` | 改 name/description/modelDefault；`[v0.0.33.4]` **enableHeartBeat / budget / timezone 真生效**（写后 `scheduler.reloadSquad()`）；`[v0.0.270]` **enableGroupChat**（群聊可见性）；`[v0.0.347]` **modelRoutingPlanId**（模型路由方案挂载） | `PatchSquadBody` | `200` + `SquadDetail`（必含 enableHeartBeat / budget / timezone / enableGroupChat 字段回显；modelRoutingPlanId 无字段省略） |
...
interface PatchSquadBody {
  ...
  description?: string;
  modelDefault?: string;
  // [v0.0.347] 模型路由方案挂载（引用 model_routing_plans 方案库 planId）。undefined=不修改；null=解除挂载（清空字段）；非空须指向存在的方案否则 400 `plan not found: <planId>`
...
**错误**：`400` body 非法 / timezone 非 IANA / budget.limit<0 / **[v0.0.116] heartbeatConfig.activeWindows 段间重叠 或 单段 start>=end（跨0点）或格式错 / interval 非 5\|15\|30\|60 / scope.mode 非法** / **[v0.0.279] effortDefault 非 'default'\|'low'\|'high'\|'max'（字段级校验，先于 404）** / **[v0.0.347] modelRoutingPlanId 非空但指向不存在的方案（400 `plan not found: <planId>`）**；`404` squad 不存在。
