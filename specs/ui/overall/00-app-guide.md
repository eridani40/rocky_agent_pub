...
- **群聊开关入口（v0.0.270，自动工作 tab）**：首页 → 「自动工作」tab → **自主性 toggle 下方「群聊开关」**（enableGroupChat，默认开）。关 = 群聊可见性关闭：agents 不再注入 SquadChat + 坐席卡「群聊」按钮隐藏 + `send_message('squadchat')` 报错（成员仅私聊）；squad 实体/session 恒存在，重开即时恢复。组件契约 `studio-page/component-group-chat-toggle.md`。
