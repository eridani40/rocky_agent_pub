...
- **聊天消息链接点击分发（三处 chat 页共享：playground / studio 单聊·群聊 / academy 版本会话）**：agent 回复气泡里的 markdown 链接 `[文本](target)` 可点击，按 target 类型分流——`http(s)`/`mailto:` 等 web scheme → **系统默认浏览器**打开（不在 app 内导航）；本地路径 → **与右侧文件区行为永远一致**（v0.0.280 老板铁律，共享 `openLocalPath` 分发 lib）：`.url` 快捷方式 → 嗅探内容提取 URL → 浏览器打开（失败降级 txt editor）；6 格式图片（png/jpg/jpeg/gif/webp/svg）→ **app 内只读图片查看弹层**；12 格式文本（`.md`/`.json`/`.jsonl`/`.yaml`/`.xml`/`.toml`/`.csv`/`.tsv`/`.txt`/`.ini`/`.env`/`.log` + 编程语言）→ **app 内预览区 tab**（**[v0.0.320] 弹层退役：三栏布局中间栏，多 tab 横滑 + 内嵌查看/编辑 + dirty 守卫 + 409 冲突检测**；workspace 相对路径走 HTTP `readWorkspaceFile` 带 version、绝对路径/`~`/`file://` 走 Electron IPC；保存冲突 → 取消=重载 / 覆盖=force；v0.0.280 覆盖 v0.0.253「强制只读」决策；**v0.0.286 起 `.md` 内 `![alt](url)` block 级独立行图片自动渲染——网络图直渲 / 本地相对路径按 md 文件目录 resolve（workspace HTTP 或 absolute IPC 读 base64）/ `data:image/` 白名单放行；点击图片全屏放大预览，Esc/遮罩/✕ 关闭**；**[v0.0.339] 例外分流：`csv`/`tsv` 无条件系统打开（不 stat、不内置，任何大小——表格文件交 Numbers/Excel）+ 其余文本 stat 大小判定（workspace→HTTP `GET /workspace/stat?path=` / absolute→Electron `shell:stat` IPC；`TEXT_OVER_SIZE_BYTES = 5MB`，`>5MB` → 系统打开 / `≤5MB` → 内置 tab；stat 失败 undefined 降级内置；图片不 stat）**）；其余本地路径（pdf/未知等）→ **系统默认应用**打开；`javascript:` 等危险协议降级纯文本不可点。LLM 已被 system prompt 引导用链接语法输出路径/URL，故 agent 提到的文件一般都能直接点开。组件契约 `chat-page/section-preview-area.md` + `chat-page/component-workspace-panel.md §4.4`；共享分发 lib `lib/open-local-path.ts`（≡ 右侧文件区）。
- **聊天滚动引导气泡（三处 chat 页共享：playground / studio 单聊·群聊 / academy 版本会话）**：消息流**不在最底部**时（用户向上翻历史），输入框上方浮动显示引导气泡——LLM **生成中**显示「新消息」、**空闲/已结束**显示「回到底部」；点击气泡**平滑滚到底部**（滚动完成后气泡消失）。用户在底部时**不显示**气泡（自动跟随流式内容滚动，不被甩开）；空会话也无气泡（走既有空态引导）。气泡浮动不占文档流——出现/消失不推动消息流/输入区布局。组件契约 `chat-page/component-scroll-guide-bubble.md` + `chat-page/_overview.md §4.5`。
- **聊天输入草稿缓存（v0.0.267，三处 chat 页共享：playground / studio 单聊·群聊 / academy 版本会话）**：在某个会话输入区输入了**一半内容**（纯文本 / mention pill / 混合 / 多行）→ 切到其他会话再切回，**输入内容完整恢复**（继续编辑不受影响）。切回时**草稿优先于外部预填**（如 studio 看板 @ 入口跳转的模板预填——有未完成草稿则恢复草稿、不覆盖）。**发送后草稿清除**（已发送内容不残留，切回输入区为空）。纯内存级——关闭应用/刷新页面后丢失（非持久化）。组件契约 `chat-page/chat-composer.md`「输入草稿缓存」节。
- **会话列表排序 + 置顶**：列表 = 置顶组在前、非置顶组在后，**同组内按更新时间倒序**（最新在上）；新建会话 / 发消息对话 / 置顶切换后列表**立即自动归位**，无需手动刷新。**置顶操作（双入口）**：① hover 会话项 → 右侧出现 **pin 按钮**（hover 显隐，已置顶常驻 accent 色）→ 点击切换置顶；② 右键会话项 → 菜单「置顶」（在「复制 Session ID」之上；已置顶则显示「取消置顶」）。置顶 → 该会话进列表顶部置顶组，item 背景加重区分；取消置顶 → 回到非置顶组按原对话时间归位（可能不在顶部）。置顶状态跨 app 重启保留。
- **右侧 workspace 面板（ws-panel，3 处 chat 页共享：playground / academy 版本会话 / studio 单聊）**：文件树浏览（**文件按自然序排列——文件夹置顶 + 同组内数字段按数值比较，如 `90.txt` 排在 `100.txt` 前；顶层与子目录同规则；SSE 文件变化刷新后顺序不变**）+ **工作区搜索框（v0.0.320：输入防抖 300ms，前端过滤已加载树 + 后端递归全量补全，结果项点文件开预览 tab / 点文件夹展开收起）** + 切换工作目录 + 刷新；**空文件夹感知**：空文件夹自身无 twisty 不可展开，但**内部新增文件/子文件夹会被实时感知**（关注集合覆盖打开节点的一级子文件夹含空），无需手动刷新即可展开看到新内容；**点文件 → 进预览区 tab（v0.0.320 起，弹层退役；playground/studio 有 PreviewArea Provider → 中间栏多 tab 打开；academy 无 Provider → 降级弹层）**——内置 viewer/editor：默认查看渲染——`.md` 走 markdown 渲染（v0.0.286 起 markdown 内 `![alt](url)` block 级独立行图片**自动渲染**：网络图 `http(s)://` 直渲 / 本地相对路径按 md 文件目录 resolve → workspace IPC 读 base64 / 绝对路径 IPC 读 base64；**点击图片弹出全屏放大预览**（Esc/遮罩/✕ 关闭）；`data:image/` 白名单放行，`javascript:`/`vbscript:`/非 image data: 拦截降级纯文本） / 结构化 7 格式（json/jsonl/yaml/xml/toml/csv/tsv）走 `<pre>` 朴素预览并可在 edit 模式显「格式化」「校验」两按钮 / 编程语言走 `<pre>` 纯文本（v0.0.320 code 分类，无高亮无格式按钮） / plain（txt/ini/env/log）走 `<pre>`；**编辑 + 保存（v0.0.320 冲突检测）**：读带 version，保存带 expectedVersion 校验，外部改动 → 409 冲突 modal（取消=重载 / 覆盖=force）；dirty 守卫（切/关/开新 tab 未保存 → 保存并切换/放弃/取消）；**`.url` 快捷方式文件**（内容指向 http/https）→ **系统浏览器打开**目标 URL（不进 editor）；**文件夹**（含 symlink→dir）→ 点击展开-收起 / hover「打开文件夹」走 `POST /workspace/open` 系统文件管理器打开目标目录。**symlink 节点浏览（v0.0.263）**：文件树显示 symlink 节点（专用 link 角标 + hover tooltip `→ 目标路径`）；点击 symlink→dir 的 twisty 可**逐层展开浏览**目标目录内容（用户显式浏览 = 授权；未授权越界路径仍拒绝）。组件契约 `section-preview-area.md` + `component-workspace-panel.md §4.4/§4.5/§4.6/§6.5` + `common/component-modal-md-editor.md`（academy 降级）。
...
### 3.2 Studio（团队 squad）
- **路径**：nav-rail「Studio」→ 左 sidebar squad 列表（两行卡片：32×32 彩色字母头像 + 团队名 + 「X 在线 · Y 工作中」状态行；Y>0 时「工作中」数字旁橙色脉冲圆点）→ 「+ 新建」建团队 → 选 squad 落**首页面板**。
- **sidebar 活跃排序 + 置顶（v0.0.305）**：列表默认按**最后活跃时间倒序**（最新活跃的 squad 排前面，实时 SSE 推送更新）。**置顶操作**：hover squad 行 → 行右侧出现 **pin 按钮** → 点击置顶 → 该 squad 跳到列表最前（置顶组整体最前，组内仍按活跃排序）；再点取消置顶 → 回到按活跃排序的位置。置顶状态存 localStorage（`studio.squadPins`），**刷新/重启保持**；点击 pin 不触发选中 squad。旧后端（无聚合字段）降级：排序按 updatedAt、第二行不渲染、不 crash。
- **首页三 tab**（内联切换）：首页（队长 mini 卡 + TokenWidget 图文 + 学生 roster + 第二栏内嵌全景）/ 管理 / 自动工作。
- **群聊开关入口（v0.0.270，自动工作 tab）**：首页 → 「自动工作」tab → **自主性 toggle 下方「群聊开关」**（enableGroupChat；**[v0.0.340] 新建团队默认关**，存量 squad 默认开）。关 = 群聊可见性关闭：agents 不再注入 SquadChat + 坐席卡「群聊」按钮隐藏 + `send_message('squadchat')` 报错（成员仅私聊）；squad 实体/session 恒存在，重开即时恢复。组件契约 `studio-page/component-group-chat-toggle.md`。
...
- **会话页成员状态入口（v0.0.268 起，v0.0.269 挪位）**：单聊/群聊落地页 **chat 右上浮菜单第 4 项「团队状态」**（squad 图标 + running badge，位置 skills 与 todo 之间、**todo 保持最后**；仅 studio 单聊/群聊显示——playground/academy 无 squad Provider 不显示）。badge = 当前 **running 成员数，含 leader**（suspended 不计；**0 running 时不显示数字仅图标**；数字随 SSE 实时刷新）。点击 → 弹出**成员状态弹层**（L3 modal）：**上区 running / 下区 idle** 分区（deployed 成员，benched 不显示），行内 = 头像 + 名字 + role 标识 + **presence 工作标记**（当前工作文字，空则「运行中/在线」fallback）；**hover 成员行出现「进入对话」icon**（无文字），点击直达该成员会话（**当前会话所属成员行不显示 icon**——防套娃，用户已在其中）。**两级导航固化**：squad 首页（坐席+会话列表）↔ 会话落地页——**从任何会话（含面板进入的）点返回键恒回 squad 首页**，不逐级回退。组件契约 `studio-page/component-squad-status-modal.md`；入口 `chat-page/component-chat-float-menu.md`。
- **workspace 图片点击（v0.0.269）**：三处 chat 页（playground / studio / academy）workspace 文件树点开 6 格式图片（png/jpg/jpeg/gif/webp/svg）→ **只读图片查看弹层**（`component-ws-image-viewer`，img 完整渲染 + 标题/副标题 + 遮罩/Esc 关闭，无编辑/保存按钮）；其余文件（文本 12 格式）进内置 editor、`.url` 浏览器打开、未知/非 6 格式图片系统打开。
