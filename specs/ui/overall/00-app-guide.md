...
- **聊天消息链接点击分发（三处 chat 页共享：playground / studio 单聊·群聊 / academy 版本会话）**：agent 回复气泡里的 markdown 链接 `[文本](target)` 可点击，按 target 类型分流——`http(s)`/`mailto:` 等 web scheme → **系统默认浏览器**打开（不在 app 内导航）；本地路径 → **与右侧文件区行为永远一致**（v0.0.280 老板铁律，共享 `openLocalPath` 分发 lib）：`.url` 快捷方式 → 嗅探内容提取 URL → 浏览器打开（失败降级 txt editor）；6 格式图片（png/jpg/jpeg/gif/webp/svg）→ **app 内只读图片查看弹层**；12 格式文本（`.md`/`.json`/`.jsonl`/`.yaml`/`.xml`/`.toml`/`.csv`/`.tsv`/`.txt`/`.ini`/`.env`/`.log`）→ **app 内弹可编辑 viewer**（modal，有「编辑/保存」按钮，workspace 相对路径走 HTTP、绝对路径/`~`/`file://` 走 Electron IPC；保存 last-write-wins + toast「已保存」；v0.0.280 覆盖 v0.0.253「强制只读」决策；**v0.0.286 起 `.md` 内 `![alt](url)` block 级独立行图片自动渲染——网络图直渲 / 本地相对路径按 md 文件目录 resolve（workspace HTTP 或 absolute IPC 读 base64）/ `data:image/` 白名单放行；点击图片全屏放大预览，Esc/遮罩/✕ 关闭**）；其余本地路径（pdf/代码等）→ **系统默认应用**打开；`javascript:` 等危险协议降级纯文本不可点。LLM 已被 system prompt 引导用链接语法输出路径/URL，故 agent 提到的文件一般都能直接点开。组件契约 `chat-page/component-chat-link-viewer.md` + `common/component-modal-md-editor.md`；共享分发 lib `lib/open-local-path.ts`（≡ 右侧 `component-workspace-panel.md §4.4`）。
...
- **右侧 workspace 面板（ws-panel，3 处 chat 页共享：playground / academy 版本会话 / studio 单聊）**：文件树浏览（**文件按自然序排列——文件夹置顶 + 同组内数字段按数值比较，如 `90.txt` 排在 `100.txt` 前；顶层与子目录同规则；SSE 文件变化刷新后顺序不变**）+ 切换工作目录 + 刷新；**空文件夹感知**：空文件夹自身无 twisty 不可展开，但**内部新增文件/子文件夹会被实时感知**（关注集合覆盖打开节点的一级子文件夹含空），无需手动刷新即可展开看到新内容；**点文件 → 本地文件一律进内置 viewer/editor 弹层**（v0.0.263 起不限扩展名，含普通 + symlink 文件；modal：默认查看渲染——`.md` 走 markdown 渲染（v0.0.286 起 markdown 内 `![alt](url)` block 级独立行图片**自动渲染**：网络图 `http(s)://` 直渲 / 本地相对路径按 md 文件目录 resolve → workspace IPC 读 base64 / 绝对路径 IPC 读 base64；**点击图片弹出全屏放大预览**（Esc/遮罩/✕ 关闭）；`data:image/` 白名单放行，`javascript:`/`vbscript:`/非 image data: 拦截降级纯文本） / 结构化 7 格式（json/jsonl/yaml/xml/toml/csv/tsv）走 `<pre>` 朴素预览并可在 edit 模式显「格式化」「校验」两按钮——格式化成功替换内容 + ✓ 提示、校验失败显「第 N 行: 错误」，**不阻塞保存** last-write-wins；点「保存」落盘 + 底部 toast「已保存」；**二进制/图片文件**（如 `.png`）显示「二进制文件无法预览」占位提示）；**`.url` 快捷方式文件**（内容指向 http/https）→ **系统浏览器打开**目标 URL（不进 editor）；**文件夹**（含 symlink→dir）→ 展开-收起 / hover 打开走 `POST /workspace/open` 系统文件管理器打开目标目录。**symlink 节点浏览（v0.0.263）**：文件树显示 symlink 节点（专用 link 角标 + hover tooltip `→ 目标路径`）；点击 symlink→dir 的 twisty 可**逐层展开浏览**目标目录内容（用户显式浏览 = 授权；未授权越界路径仍拒绝）。组件契约 `component-workspace-panel.md §4.4/§4.5/§6.5` + `common/component-modal-md-editor.md`。
