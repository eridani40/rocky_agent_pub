...
## 职责
角色面板：编辑某 member 配置，**占用主区**（非弹层/抽屉），左上「返回」回**首页 seats**。section 纵向：姓名介绍（name + intro + **workStyle 多行**）/ **skills**（inherit/custom switch + 简化筛选器）。**当前任务占位区删除**。**心跳配置 section 移除**——心跳升级 squad 级统一调度。改动后**底部统一保存条 SaveBar**（`component-save-bar.tsx`，sticky bottom-0，替代原右下 fixed 悬浮按钮）：`dirty` 时亮起，PATCH member 保存；保存成功后基线重置 → dirty 清 → SaveBar 熄灭；「取消」= reset（draft 回基线）。
...
- 保存仅传改动字段；`tools` accept-and-ignore）；成功后基线重置 → dirty 清 → SaveBar 熄灭。
- **[v0.0.317] 底部统一保存条**：`<SaveBar dirty={dirty} saving={saving} onSave={save} onCancel={reset} />`（sticky bottom-0，`component-save-bar.tsx`）。save 守卫 `!dirty || saving` 短路；reset = draft 回基线。
