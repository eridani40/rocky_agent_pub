...
| 单聊渲染策略：对端固定 actor（a2a 取 `sender.agent.ref` 非 member）+ **a2a→右**（与 user 同侧，区别于群聊 a2a→左） | `chat-page/chat-actor-strategy.tsx`（`resolveMemberActorFactory` / `memberSideResolver`，逐行等价迁移 + UT 锁定）；策略选择由 `deriveRenderStrategy(chrome)` 按 `chrome.memberId` 数据驱动 |
...
设计动机（单聊 a2a 为何→右）：单聊主轴是 user ↔ member 对话，a2a 是第三方旁路投递——视觉与 user 同侧，避免与 member answer（左）混淆。
