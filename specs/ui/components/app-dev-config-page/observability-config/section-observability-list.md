...
## 职责
可观测性列表视图：标题区 + provider-card 列表 + 「添加配置」卡 + 删除确认 modal。每项点击进入详情；启停 toggle 即时；删除经 modal 二次确认。
...
- onToggle: (id: string, enabled: boolean) => void;  // toggle 即时生效
- onDelete: (id: string) => void;       // modal 确认后调
