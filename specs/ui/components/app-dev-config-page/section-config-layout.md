...
## 职责
三栏 config 布局（app/dev/合并设置页共用）：左 group 列表（自渲染以匹配 ET 锚点 `group-item-{groupId}`）+ 右 配置区（KV 模式 = 每 key 一张 `component-key-card`；或 page 注入的自定义节点）+ 当前 group 的独立保存条（`component-group-save-bar`，仅 `saveMode='group'` 渲染）。
