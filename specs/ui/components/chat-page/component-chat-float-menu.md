...
> （收纳原 ws-panel「长期记忆」「定时任务」tab → 悬浮菜单 + 弹层；[v0.0.205.t2_cons] 加第 3 菜单项「skills」；[v0.0.223] 加第 4 菜单项「待办 todo」；[v0.0.269] 加第 5 菜单项「团队状态」；**[v0.0.356] 加第 6 菜单项「余额查询 quota」——squad-status 后、todo 前，todo 仍保持最后 → 共 6 项**）
...
## 1. 定位 + 设计意图（一句话）
聊天区右上方（贴 chat-detail 与 ws-panel 分界处、topbar 下方）一个竖向圆角白色悬浮工具条，纵排「长期记忆」「定时任务」「skills」「团队状态」「余额查询」「待办」六个小图标菜单项（memory/cron/squad-status 带 badge 计数；skills/quota 无 badge——无计数需求；**[v0.0.356] quota 在 squad-status 下方、todo 上方**）。顺序自上而下 = memory/cron/skills/squad-status/quota/todo（v0.0.269：squad-status 在 skills 下方、todo **上方**；**v0.0.356：quota 新插入 squad-status 与 todo 之间；todo 仍保持最后**）。点菜单项弹出对应弹层——memory/cron 弹层内「列表 ↔ 新建/编辑」走**二级视图导航（顶部返回按钮）**，不再弹层套弹层；skills 弹层为 3 tab 只读列表（见 `component-skills-modal.md`）；todo 弹层为双层树只读视图（见 `component-todo-modal.md`）；squad-status 弹层为成员状态分区只读视图（见 `component-squad-status-modal.md`）。三处 chat 页统一复用（由 `component-chat-right-overlay` 承载，见 §8）。
...
## 7. 状态 / 交互（关键约束）
- 菜单项点击 → 设 `openModal: 'memory' | 'cron' | 'skills' | 'squad-status' | 'quota' | 'todo' | null`（float-menu 持有）→ 挂对应弹层。
...
- 关闭弹层（遮罩点击 / 关闭按钮）→ `openModal=null`；重开回列表态。**[v0.0.356] quota 弹层由 `ComponentQuotaEntryModal` 内部自持开/关，float-menu 不再持有 quota 弹层状态（quota 无 badge，开即拉关即停）**。
- 数据 hook（memory/cron/skills/todo）恒挂载于 float-menu（不随弹层开关 mount/unmount）→ 弹层开关不触发重 GET，badge 常驻数据；skills 弹层每次打开由弹层侧调一次 `catalog.refetch()`（PRD UC-S7 重开刷新）；**[v0.0.228] todo 弹层同例**——每次打开由弹层侧调一次 `crud.refetch()`（打开瞬间必最新，与 SSE 实时增量互补）；**[v0.0.269] squad-status 弹层同例**——打开时弹层侧 `refreshDetail()` fire-and-forget（presence 尽量新）。**[v0.0.356] quota 弹层不恒挂载：由 `ComponentQuotaEntryModal` 内部 `useSquadQuota` 在 open 时挂载、close 时卸载，开即拉四源、关即停轮询。**
...
- 可见文案（chat ns）：菜单 aria-label `floatMenu.memory|cron|skills|squadStatus|quota|todo`（zh「长期记忆/定时任务/技能/团队状态/余额查询/待办」）。

## 复用关系
- 被组合：`component-chat-right-overlay`（右缘统一 overlay，与 `component-history-minimap`
- 组合（child，coder 前置产出 .md/.tsx）：`component-memory-modal`（记忆弹层二级视图）+ `component-cron-modal`（cron 弹层）+ `component-skills-modal`（skills 3 tab 只读弹层，v0.0.205.t2_cons）+ `component-todo-modal`（todo 双层树只读弹层，v0.0.223）+ `component-squad-status-modal`（成员状态分区只读弹层，v0.0.269，studio-page 目录）+ `component-quota-entry-modal`（余额查询 L3 弹层，v0.0.356，内部挂载 `useSquadQuota` 四源 hook）。
- **v0.0.356 quota 入口**：渲染条件 `{squadCtx?.detail?.modelRoutingPlanId && ...}`（无 Provider 或未挂载方案 → 不渲染）；点击打开 `ComponentQuotaEntryModal`（`planId={squadCtx.detail.modelRoutingPlanId}`，`onClose` 关弹层）；quota 项无 badge。
