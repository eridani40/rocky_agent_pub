...
> 组件清单: message-stream / tool-batch / tool-call-item / loading-status / empty-state / run-finish / subagent-tree / chat-link-viewer（+ `chat-link-handler-context` 纯 TS Context 断循环依赖）/ history-minimap / scroll-guide-bubble / a2a-envelope + common primitive（primitive-bubble / primitive-markdown-view）
