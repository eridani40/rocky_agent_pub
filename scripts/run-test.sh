...
# 面板能捕捉完整 event 流（排查「丢中间 message」类问题）。用法：./scripts/run-test.sh --debug
if [ "${1:-}" = "--debug" ]; then
  export ROCKY_OPEN_DEVTOOLS=1
  echo "[run-test.sh] --debug: electron devtools 将自动打开（detach），可用 Network/EventSource 观察 SSE"
fi

if [ ! -f ./test.env ]; then
  echo "[run-test.sh] ERROR: 缺失 test.env，请执行 cp test.env.example test.env（参考 specs/tech/app/envs/[P0]environments.md §3.5）" >&2
  exit 1
fi
...
for required in APP_NAME APP_ENV API_PORT WEB_PORT DATA_DIR API_START_CMD WEB_START_CMD HEALTH_ENDPOINT; do
  if [ -z "${!required:-}" ]; then
    echo "[run-test.sh] ERROR: test.env 缺关键字段 ${required}。请对照 test.env.example 补齐。" >&2
    exit 2
  fi
done

echo "[run-test.sh] APP_NAME=$APP_NAME env=$APP_ENV API_PORT=$API_PORT WEB_PORT=$WEB_PORT DATA_DIR=$DATA_DIR"
...
echo "[run-test.sh] 0a. generating app/server/app-version.json ..."
bun run gen-version
...
echo "[run-test.sh] starting server: $API_START_CMD"
sh -c "$API_START_CMD" &
SERVER_PID=$!

# 2. 起 @app/web dev server（渲染层 Vite，监听 WEB_PORT）
echo "[run-test.sh] starting web dev: $WEB_START_CMD"
sh -c "$WEB_START_CMD" &
WEB_PID=$!

# 3. 轮询 server 就绪（HEALTH_ENDPOINT），再起 electron
HEALTH_URL="http://127.0.0.1:${API_PORT}${HEALTH_ENDPOINT}"
echo "[run-test.sh] waiting server ready at $HEALTH_URL ..."
for i in $(seq 1 60); do
  if kill -0 "$SERVER_PID" 2>/dev/null && curl -fsS "$HEALTH_URL" >/dev/null 2>&1; then
    echo "[run-test.sh] server ready (after ${i}*0.5s)"
    break
  fi
  if ! kill -0 "$SERVER_PID" 2>/dev/null; then
    echo "[run-test.sh] ERROR: server 进程已退出，见日志" >&2
    exit 3
  fi
  sleep 0.5
done
...
echo "[run-test.sh] waiting web dev ready at $WEB_URL ..."
for i in $(seq 1 60); do
  if kill -0 "$WEB_PID" 2>/dev/null && curl -fsS "$WEB_URL" >/dev/null 2>&1; then
    echo "[run-test.sh] web dev ready (after ${i}*0.5s)"
    break
  fi
  if ! kill -0 "$WEB_PID" 2>/dev/null; then
    echo "[run-test.sh] ERROR: web dev 进程已退出，见日志" >&2
    exit 3
  fi
  sleep 0.5
done
...
echo "[run-test.sh] building electron TS (tsc -b) ..."
...
echo "[run-test.sh] starting electron (VITE_DEV_SERVER_URL=$VITE_DEV_SERVER_URL) ..."
(cd app/electron && bun run dev) &
ELECTRON_PID=$!

cleanup() {
  echo "[run-test.sh] shutting down (server=$SERVER_PID web=$WEB_PID electron=$ELECTRON_PID)"
...
echo "[run-test.sh] a child exited (code=$EXIT_CODE)，开始清理"
exit "$EXIT_CODE"
