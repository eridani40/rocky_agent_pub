...
**回报「完成」之前，必须执行 `git diff --stat HEAD~1`（或 `git diff --cached --stat`），确认你声称改的每个文件都在 diff 里。**

**根因**：UT 通过 ≠ 业务代码改了。mock 可以脱离业务代码独立跑通。不 commit 的改动 = 不存在。

**回报模板（MANDATORY）**：
```
## 改了什么
- `path/to/file.ts`：具体改动描述

## 自查
- `git diff --stat` 确认文件列表（贴出 stat 输出）
- UT：N/N passed
- tsc：0 error
```

**禁止**：只说「改完了」「全绿」而不贴 diff stat。**diff stat 是交付凭证**。

## 测试环境变量（MANDATORY）

运行需要 API Key 的测试时，优先使用项目中已配置的环境变量。按项目配置注入环境变量后执行测试命令。

**实际执行命令**：按项目配置的测试命令执行单元测试和类型检查（以项目 package.json scripts 为准）。worktree 里若依赖解析失败，先补 workspace 局部 node_modules symlink，不要怀疑代码。

## 运行时兼容性注意（项目运行时 vs Node）

本项目跑在 **项目运行时** 上，其内置 undici 与 Node 的 undici **API 面不同**。写涉及 undici dispatcher 的代码（proxy / `EnvHttpProxyAgent` / `Agent` / proxyFetch）时：
- 项目运行时 内置 undici（8.5.x）的 `Agent`/`EnvHttpProxyAgent` **没有 `close()` 方法**。finally 里 `await dispatcher.close()` 会在 项目运行时下抛 `dispatcher.close is not a function`，被上层 catch 包装成工具/provider 失败。
- **必须能力探测**：`if (typeof d.close === 'function') await d.close().catch(() => {})`，无则跳过（项目运行时下由 GC 回收，无泄漏）。
- 凡"Node 有但 项目运行时 可能没有"的 API，一律 `typeof` 探测，不假设完整 API 面。UT 注入 mock 时**模拟 项目运行时 的精简 API**（如 FakeAgent 不定义 close），才能抓到这类 bug。

## 打包兼容自检（MANDATORY — 改依赖/路径/plugin/启动入口时）

**dev 能跑 ≠ packaged 能跑**（packaged = Electron Node CJS + asar + cwd=`/`，不是 项目运行时）。涉及以下改动，按 AGENTS.md「持续可打包护栏」自检 + 向 orchestrator 汇报：
...
本项目用 **项目测试框架**（`globals: true`）。`describe`/`it`/`expect`/`vi` 全局可用无需 import。
...
**只能运行自己本次编写的单元测试**，不要跑全量 `按项目配置的测试命令`。用 项目测试框架 指定文件运行：
```bash
按项目配置的测试命令执行 path/to/__tests__/your-test-file.test.ts
```
