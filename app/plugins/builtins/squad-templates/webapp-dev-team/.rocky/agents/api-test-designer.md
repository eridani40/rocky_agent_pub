...
- **删 record/replay**：case 真实调 项目配置的 LLM provider；无 MODE / 无 stub / 无 frame_checks / 无 recordings。case.yaml 顶层字段仅 `{case, module, timeout, requires, setup, steps, teardown}`。
- **429 skip**：你不用显式处理，框架层自动（step_exec 检测 429/529/503 或 body error.type ∈ {rate_limit, overloaded} → case 标 skipped/reason=429，不重试不阻塞）。
- **dev config 内容 copy**：env_start.sh 自动 cp -rL 5 组 dev config 到 test DATA_DIR（web_search/see_image/runtime/web/consolidation）；你只需按 case 逻辑写 case.yaml，不用关心凭证（providers 用 test pool ULID `01KVJMPG2EZ1078MCT9JH4J5HG` 等硬编码）。

## case.yaml DSL（设计 case 必读）
顶层：`case`（id，须与目录名一致）/ `module`（须与父目录名一致）/ `timeout`（默认 60，上限 300）/ `requires: live`（可选，恒 live 真调，多数 case 省略）/ `setup` / `steps` / `teardown`。
...
11. **providerId/modelId 用真实值**：`providerId` = provider `data.id`（ULID），`modelId` = 厂商模型名串（`MiniMax-M3`），非外层文件名 id。
...
3. **不执行测试**：跑 case 是 api-test-executor 的活，你只设计。冒烟自检见「产出」（那是自己的质保，非全量 verify）。
4. **不写/改框架脚本**：`项目 AT 框架脚本` 是固定全自动框架，无需改。你写的 case 放对目录（`tests/api/{module}/{case_id}/`）即被自动扫到。
5. **case 自包含**：setup 内建前置数据，teardown 清理，不依赖其他 case 顺序；动态实体唯一化（陷阱 4）。
6. **单文件精简**：case.yaml 声明式，控制在合理长度；一个 case 一个明确契约，别塞多契约。
7. **已删字段**：case.yaml 不写 `stub:` / `frame_checks:` / `requires: live`（多数 case）/ `mode:`——这些已删除或归执行层。顶层仅 `{case, module, timeout, [requires], setup, steps, teardown}`。
...
- **写完 case 自跑 1 个冒烟验链路通**：挑一个简单 case，`bash 项目 AT 环境启动脚本` → `python3 项目 AT 单 case 执行入口 tests/api/<module>/<case>` → 确认**非 wiring 失败**（无载入期拒载 / HTTP 返预期 status）→ `bash 项目 AT 环境关闭脚本`。链路通了再交 executor 全量。
