...
import { readPersistedInstances, unpersistInstance, killProcessGroupByPid, errMsg } from './instance-record';
...
/** 可注入依赖（registry 必填；portBusy/now/idle/attach/reconcile 可选） */
...
/** key = `${sessionId}:${mode}[:${profileName}]`（owner 天然隔离） */
export function instanceKey(sessionId: string, opts: BrowserLaunchOptions): string {
  return opts.mode === 'managed-profile' && opts.profileName
    ? `${sessionId}:${opts.mode}:${opts.profileName}`
    : `${sessionId}:${opts.mode}`;
}

/** mode 展示文本（"headless" / "managed-profile p1"），错误提示复用 */
function modeLabel(opts: BrowserLaunchOptions): string {
  return opts.mode + (opts.profileName ? ' ' + opts.profileName : '');
}
...
export class BrowserInstanceManager {
  ...
  /** 开机自检：读记录 → 按 rec.mode 查 registry → impl.cleanupOrphan */
  private cleanupOrphans(): void {
    let records: PersistedInstanceRecord[] = [];
    try {
      records = readPersistedInstances(this.dataDir);
    } catch (e) {
      console.warn(`[browser-instance-manager] 开机自检读记录失败: ${errMsg(e)}`);
      return;
    }
  ...
  private async reconcileOrphans(): Promise<void> {
    ...
    let records: PersistedInstanceRecord[] = [];
    try { records = readPersistedInstances(this.dataDir); } catch { records = []; }
    for (const rec of records) {
    ...
    for (const proc of scan.candidates) {
      ...
      if (proc.userDataDir) {
        ...
        // 记录同步：孤儿 chrome 的 rocky 目录匹配记录 → unpersist（防记录残留下次启动误判）
        for (const rec of records) {
          if (rec.userDataDir === proc.userDataDir) unpersistInstance(this.dataDir, rec.key);
        }
      }
    }
  }
  ...
  /** 显式启动/复用。幂等：ready 复用；非 ready → 清理旧 + impl.launch 新建 */
  async launch(sessionId: string, opts: BrowserLaunchOptions): Promise<BrowserExecuteResult> {
    const key = instanceKey(sessionId, opts);
    const existing = this.instances.get(key);
    if (existing?.state === 'ready') { existing.lastUsedAt = this.nowFn(); return { ok: true, text: `reuse ${modeLabel(opts)}` }; }
    if (existing) {
      try { await this.closeInstance(key, existing); } catch (e) {
        console.warn(`[browser-instance-manager] launch 前清理旧实例失败 ${key}: ${errMsg(e)}`);
      }
    }
    const r = await this.registry.get(opts.mode).launch(key, opts, this.env);
  ...
  /** 单实例关闭：registry 分发 impl.close（资源清理全在 impl）+ 删表 */
  private async closeInstance(key: string, inst: BrowserHandle): Promise<void> {
    try { await this.registry.get(inst.mode).close(inst, this.env); } catch (e) {
