...
/**
 * 合法 topic 集合（agent_loop + session_panel + session_meta + app_task + panorama）。
 * 与 bootstrap-bus-phase.ts 的 registerTopic 清单一一对应（两处手维护；
 * __tests__/sse-topic-whitelist.test.ts 有双向对齐断言防漏配）。
 */
export const ALLOWED_TOPICS = new Set(['agent_loop', 'session_panel', 'session_meta', 'app_task', 'panorama']);
