...
// xdotool key spec → Carbon virtualKey 映射（'a'/'Return'/'super+c'/'Up'/'KP_0'）。
// 从旧 swift-helper 逐字迁入 addon 包（iFurySt open-codex KeyMapping 的 xdotool alias 全表）。
// InputSimulation.pressKey 依赖 KeyPressParser.parse；随 InputSimulation 一并迁入以编译闭合。
...
enum KeyPressParser {
    ...
    private static let keyCodeMap: [String: CGKeyCode] = [
        ...
        "kp_enter": CGKeyCode(kVK_ANSI_KeypadEnter), "kp_equal": CGKeyCode(kVK_ANSI_KeypadEquals),
        "kp_multiply": CGKeyCode(kVK_ANSI_KeypadMultiply), "kp_add": CGKeyCode(kVK_ANSI_KeypadPlus),
        "kp_subtract": CGKeyCode(kVK_ANSI_KeypadMinus), "kp_decimal": CGKeyCode(kVK_ANSI_KeypadDecimal),
        "kp_divide": CGKeyCode(kVK_ANSI_KeypadDivide), "kp_delete": CGKeyCode(kVK_ANSI_KeypadDecimal),
    ]
}
