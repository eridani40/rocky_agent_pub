/**
 * SoftwareCursorGlyphRenderer：光标 glyph 的 CoreGraphics 绘制（雾 + pointer）。
 *
 * 逐字对齐 open-codex `SoftwareCursorGlyphRenderer.swift`（v0.0.160 gap#11）。
 * Rocky 侧适配：
 *   - **参考位图加载路径删除**（open-codex 依赖 docs/references/... PNG 资源，Rocky 无此 asset）
 *     → `SoftwareCursorGlyphRenderer.draw` 直接走 procedural 绘制分支；`loadReferenceCursorWindowImage`
 *     恒返 nil，保 `state.appKitDrawingState` 后续路径原逻辑不变。
 *
 * 参考: refs/open-codex-computer-use/.../SoftwareCursorGlyphRenderer.swift
 */
...
/// 光标 glyph 一帧渲染状态：由 CursorVisualDynamicsAnimator 派生。
struct SoftwareCursorGlyphRenderState {
    ...
    /// 转 AppKit 绘制坐标系（y 向上翻转 + rotation 取反）。
...
/// 光标 glyph 尺寸常量（对齐 open-codex 参考实现）。
...
/// 光标 glyph 绘制器（雾 + pointer，procedural 或参考位图）。
enum SoftwareCursorGlyphRenderer {
    /// 参考位图缓存（Rocky 无 asset → 恒 nil，走 procedural 分支）。
    private static let referenceImage: NSImage? = loadReferenceCursorWindowImage()

    /// 绘制一帧光标 glyph 到给定 context。
    static func draw(
        in bounds: CGRect,
        context: CGContext,
    ...
    /// 绘制 pointer 三角形（procedural 轮廓 + 阴影 + 描边）。
    ...
    /// 光标 pointer 轮廓路径（30 行等高线，映射到给定 rect）。
...
/// Rocky 无参考位图 asset → 恒返 nil，走 procedural 绘制分支。
/// - Note: 若未来打包 asset，可改此函数从 `Bundle.main` 查资源。
